"""
================================================================================
Параллельная равномерная сетка (без адаптивного измельчения)
================================================================================
Запуск:  python 31_parallel_grid.py

Особенности:
- Равномерная сетка mu × L, каждая точка считается независимо
- Параллелизация через multiprocessing.Pool
- Прогресс-бар tqdm с ETA
- Checkpoint каждые N точек
- По умолчанию: 50×50, g=-1
================================================================================
"""
import os
import sys
...
# ==============================================================================
# Omega_ren с LRU-кэшем (копия из 27_adaptive)
# ==============================================================================
...
def find_minimum(mu, L, g, M_max=10.0, b_max=10.0, maxiter=15, popsize=5):
    obj = lambda x: Omega_ren(x[0], x[1], mu, L, g)
    result = differential_evolution(
        obj, bounds=[(0.0, M_max), (0.0, b_max)],
        maxiter=maxiter, popsize=popsize, polish=False, tol=1e-6
    )
    M_opt, b_opt = result.x
    val_opt = result.fun

    # Канонизация: если M≈0 → b=0 (физически не зависит от b)
    M_canon_tol = 0.05
...
def _init_worker(g):
    _worker_cache['g'] = g
    find_minimum(4.0, 1.0, g)  # прогрев Numba JIT
...
if __name__ == '__main__':
    ...
    L_min, L_max = 0.1, 4.0
    n_mu = 50           # разбиение по mu
    n_L = 50            # разбиение по L
    n_workers = 12       # число процессов
    ...
    print(f"{'='*60}")
    print(f"Равномерная сетка: {n_mu}×{n_L} = {total} точек")
    print(f"Workers: {n_workers} | ETA: ~{total * 0.13 / n_workers / 60:.1f} мин")
    print(f"{'='*60}\n")
    ...
    with Pool(n_workers, initializer=_init_worker, initargs=(g,)) as pool:
        for i, (mu, L, M_min, b_min, val) in enumerate(tqdm(
            ...
            if (i + 1) % checkpoint_every == 0 or (i + 1) == total:
                ...
                np.savez(os.path.join(out_dir, '31_grid_ckpt.npz'),
    ...
    ax.set_title(f'$M_{{min}}(\mu, L)$, g={g} ({n_mu}×{n_L} grid)')
    ...
    ax.set_xlabel(r'$L$', fontsize=12)
    ax.set_ylabel(r'$\mu$', fontsize=12)
    ax.set_title(f'$b_{{min}}(\mu, L)$, g={g} ({n_mu}×{n_L} grid)')
    fig.colorbar(im, ax=ax, label=r'$b_{min}$')

    plt.tight_layout()
    png_path = os.path.join(out_dir, '31_grid_final.png')
