"""
Адаптивная диаграмма 20x20 base + max_depth=2 с checkpointing.
Сохраняет .npz и .png каждые checkpoint_every уникальных точек.
Использует differential_evolution (DE) для надежного поиска минимума.
"""
import os
import sys
import numpy as np
...
def find_minimum(mu, L, g, M_max=10.0, b_max=10.0, maxiter=10, popsize=4):
...
# ==============================================================================
class AdaptivePhaseDiagramCheckpoint:
    def __init__(self, g, mu_range, L_range, max_depth=2,
                 eps_M=0.3, eps_b=0.2, M_max=10.0, b_max=10.0,
                 init_mu_div=20, init_L_div=20,
                 checkpoint_every=250, out_dir='output'):
        self.g = g
    ...
    def save_checkpoint(self, final=False):
        ...
        npz_path = os.path.join(self.out_dir, f'15_adaptive_{tag}_g_m1_0.npz')
        np.savez(npz_path,
        ...
        png_path = os.path.join(self.out_dir, f'15_adaptive_{tag}_g_m1_0.png')
        self._plot(png_path)
...
def main():
    ...
    diagram = AdaptivePhaseDiagramCheckpoint(
        g=-1.0,
        mu_range=(0.0, 7.0),
        L_range=(0.1, 10.0),
        max_depth=2,
