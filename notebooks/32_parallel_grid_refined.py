"""
================================================================================
Параллельная равномерная сетка (mu,L) + двухэтапный поиск минимума (M,b)
================================================================================
Этап 1: grid search N×N по (M,b) — находим ячейку с минимумом
Этап 2: DE в окрестности ±шаг_сетки — уточнение

Преимущество: grid гарантирует попадание в глобальную яму,
             DE на 2-м этапе работает в маленьком окне → быстрее сходится.

Запуск:  python 32_parallel_grid_refined.py
================================================================================
"""
import os
import sys
...
# ==============================================================================
# Omega_ren (копия из 31_parallel_grid.py)
# ==============================================================================
...
# ==============================================================================
# Двухэтапный поиск минимума
# ==============================================================================
def find_minimum_grid_refined(mu, L, g, M_max=10.0, b_max=10.0,
                               grid_M=10, grid_b=10):
    """
    Этап 1: grid search grid_M × grid_b по (M,b)
    Этап 2: DE в окне ±шаг_сетки вокруг лучшей ячейки
    """
    # --- ЭТАП 1: грубый grid ---
    ...
    # --- ЭТАП 2: DE в окрестности лучшей точки ---
    ...
    # Если лучшая точка на границе — расширяем окно
    if best_M == 0.0: M_high = min(M_max, 2 * dM)
    if best_b == 0.0: b_high = min(b_max, 2 * db)
    
    result = differential_evolution(
        lambda x: Omega_ren(x[0], x[1], mu, L, g),
        bounds=[(M_low, M_high), (b_low, b_high)],
        maxiter=8,          # меньше итераций — окно маленькое
        popsize=4,          # меньше популяция
        polish=False,
        tol=1e-6
    )
    ...
    # Канонизация высокой фазы
    M_canon_tol = 0.1
    if M_opt < M_canon_tol:
        M_opt = 0.0
        b_opt = 0.0
    elif M_opt < 1e-3:
        # Дополнительная строгая канонизация
        M_opt = 0.0
        b_opt = 0.0
    else:
        # Если при найденном M разница с b=0 пренебрежимо мала — минимум в b=0
        val_b0 = Omega_ren(M_opt, 0.0, mu, L, g)
        if abs(val_opt - val_b0) < 1e-5:
            b_opt = 0.0
        elif b_opt < 1e-4:
            b_opt = 0.0
...
def _init_worker(g):
    _worker_cache['g'] = g
    find_minimum_grid_refined(4.0, 1.0, g)  # прогрев JIT

def _compute_one(args):
    mu, L = args
    g = _worker_cache.get('g', -1.0)
    M_min, b_min, val = find_minimum_grid_refined(mu, L, g)
...
if __name__ == '__main__':
    ...
    L_min, L_max = 0.1, 4.0
    n_mu = 100           # разбиение по mu
    n_L = 100            # разбиение по L
    n_workers = 10       # число процессов
    ...
    print(f"{'='*60}")
    print(f"Grid-refined parallel: {n_mu}×{n_L} = {total} точек")
    print(f"Workers: {n_workers} | ETA: ~{total * 0.12 / n_workers / 60:.1f} мин")
    print(f"Этап 1: grid 10×10 | Этап 2: DE (maxiter=8, popsize=4)")
    print(f"{'='*60}\n")
    ...
    with Pool(n_workers, initializer=_init_worker, initargs=(g,)) as pool:
        for i, (mu, L, M_min, b_min, val) in enumerate(tqdm(
            ...
            if (i + 1) % checkpoint_every == 0 or (i + 1) == total:
                ...
                np.savez(os.path.join(out_dir, '32_grid_refined_ckpt.npz'),
    ...
    ax.set_title(f'$M_{{min}}(\mu, L)$, g={g} (grid+DE, {n_mu}×{n_L})')
    ...
    ax.set_xlabel(r'$L$', fontsize=12)
    ax.set_ylabel(r'$\mu$', fontsize=12)
    ax.set_title(f'$b_{{min}}(\mu, L)$, g={g} (grid+DE, {n_mu}×{n_L})')
    fig.colorbar(im, ax=ax, label=r'$b_{min}$')

    plt.tight_layout()
    png_path = os.path.join(out_dir, '32_grid_refined_final.png')
