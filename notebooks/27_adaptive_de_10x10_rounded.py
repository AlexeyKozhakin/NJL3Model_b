"""
Финальная адаптивная диаграмма: differential_evolution БЕЗ workers
(однопоточный, чтобы избежать multiprocessing overhead на macOS).
maxiter=15, popsize=5, polish=False — DE сам находит глобальный минимум.
Если polish=True, Nelder-Mead часто скатывается в (0,0) на острых ямах.
"""
import os
import sys
import numpy as np
...
def find_minimum(mu, L, g, M_max=10.0, b_max=10.0,
    ...
    result = differential_evolution(
        ...
        maxiter=maxiter,
        popsize=popsize,
        polish=False,   # Nelder-Mead портит результат на острых ямах!
        tol=1e-6
        # workers=-1 убрано — однопоточный режим, нет multiprocessing overhead
    )
    M_opt, b_opt = result.x
    val_opt = result.fun

    # Канонизация высокой фазы (M≈0):
    # При M=0 потенциал математически не зависит от b, но численный шум
    # даёт случайное b. Если M ниже порога — принудительно ставим (0, 0).
    M_canon_tol = 0.05
...
class AdaptivePhaseDiagram:
    ...
    def save_checkpoint(self, final=False):
        ...
        npz_path = os.path.join(self.out_dir, f'27_adaptive_rounded_{tag}_g_m1_0.npz')
        np.savez(npz_path,
        ...
        png_path = os.path.join(self.out_dir, f'27_adaptive_rounded_{tag}_g_m1_0.png')
        self._plot(png_path)
    ...
    def _plot(self, out_path):
        ...
        ax.set_title(f'$M_{{min}}(\mu, L)$, g={self.g} (DE, {len(self.points_mu)} pts)')
        ...
        ax.set_xlabel(r'$L$', fontsize=12)
        ax.set_ylabel(r'$\mu$', fontsize=12)
        ax.set_title(f'$b_{{min}}(\mu, L)$, g={self.g} (DE, {len(self.points_mu)} pts)')
